...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		objectKey := order.GetFilename()
		object, err := s3.GetObject(r.Context(), storage.OrdersBucket, objectKey, minio.GetObjectOptions{})
		if err != nil {
			errResponse, isResponsError := err.(minio.ErrorResponse)
			if isResponsError && errResponse.Code == "NoSuchKey" {
				render.Status(r, http.StatusNotFound)
				render.JSON(w, r, fmt.Sprintf("Reciept file not found for order %d", uintId))
			}
			slog.Error("Unable to get receipt", slog.String("error", err.Error()), slog.String("key", objectKey))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to retrieve receipt file")
		}
		defer object.Close()
		...
		w.Header().Set("Content-Disposition", fmt.Sprintf("inline; filename=\"%s\"", objectKey))
...
func PostOrder(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		receiptReader := strings.NewReader(markdownContent)
		// Put the file into s3 using s3.PutObject(), the bucket name is defined in storage.OrdersBucket.
		// dbOrder.Filename() can be used to get the filename.
		objectKey := dbOrder.GetFilename()
		// Size of the file is determined by the string.
		contentSize := int64(len(markdownContent))
		// Use the following PutObjectOptions: minio.PutObjectOptions{ContentType: "text/markdown"}
		uploadOptions := minio.PutObjectOptions{ContentType: "text/markdown"}
		_, err = s3.PutObject(r.Context(), storage.OrdersBucket, objectKey, receiptReader, contentSize, uploadOptions)
		// Handle errors!
		if err != nil {
			slog.Error("WARNING: Failed to put receipt object to S3", slog.String("error", err.Error()), slog.String("key", objectKey))
		}
		render.Status(r, http.StatusOK)
		render.JSON(w, r, "ok")
	}
